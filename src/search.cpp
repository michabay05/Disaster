  ...
  for (int currDepth = 1; currDepth <= depth; currDepth++) {
    ...
    if (pvLength[0]) {
      ...
      if (score > -MATE_VALUE && score < -MATE_SCORE) {
        printf("info score mate %d depth %d nodes %d time %lld pv ",
               -(score + MATE_VALUE) / 2 - 1, currDepth, nodes, endTime);
      } else if (score > MATE_SCORE && score < MATE_VALUE) {
        printf("info score mate %d depth %d nodes %d time %lld pv ",
               (MATE_VALUE - score) / 2 + 1, currDepth, nodes, endTime);
      } else {
        printf("info score cp %d depth %d nodes %d time %lld pv ", score,
               currDepth, nodes, endTime);
      }
      for (int i = 0; i < pvLength[0]; i++)
        printf("%s ", Move::toString(pvTable[0][i]).c_str());
      printf("\n");
    }
  }
  printf("bestmove %s\n", Move::toString(pvTable[0][0]).c_str());
}
...
int negamax(Board *board, int alpha, int beta, int depth) {
  ...
  // table
  if (ply > 0 && (score = TT::readEntry(*board, alpha, beta, depth)) != TT::NO_ENTRY &&
      !isPVNode)
    return score;
  ...
  if (depth >= 3 && !inCheck && ply) {
    ...
    if (board->enpassant != noSq)
      Zobrist::toggleEnpass(*board, board->enpassant);
    // Reset enpassant
  ...
  for (int i = 0; i < moveList.count; i++) {
    ...
    else // Late move reduction (LMR)
    {
      if (movesSearched >= FULL_DEPTH_MOVES && depth >= REDUCTION_LIMIT &&
          !inCheck && Move::getPromoted(moveList.list[i]) == E &&
    ...
    *board = clone;
    ...
    if (score > alpha) {
      ...
      if (!Move::isCapture(moveList.list[i]))
        // Store history move
        historyMoves[Move::getPiece(moveList.list[i])]
                    [Move::getTarget(moveList.list[i])] += depth;
